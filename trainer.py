...
class Trainer(object):
    ...
    def _rollout_model_expert(self,start_states,sample_pool=None,IRL=True,Penalty_only=False):
        ...
        while t < self._reward_step:
            ...
            if self._keep_logvar:
                # 简化输出：减少冗余信息
                if t % 10 == 0:  # 每10步显示一次
                    print(f"Reward Var: mean={np.mean(reward_var.cpu().numpy()):.4f}, Remaining={np.sum(not_done)/len(not_done):.2f}")
            ...
            if not_done.sum() == 0:
                # 简化输出
                print(f"Rollouts completed after {t} timesteps")
                break
    ...
    def train_offline(self, num_epochs, save_model=False, save_policy=False, load_model_dir=None):
        ...
        # 添加总进度提示
        print(f"\\nStarting offline training for {num_epochs} epochs...")
        import time
        ...
        for i in range(num_epochs):
            ...
            # print("------------------------")
            print(f"Epoch {i+1}/{num_epochs} (Progress: {progress:.1%}, ETA: {remaining_time/60:.1f} min)")
            stats_fmt = "{:<20}{:>30}"
            stats_str = ["Epoch",
                         "WM Reward Mean",
                         "WM Reward Max",
    ...
    def test_agent(self, use_model=False, n_evals=None, population_idx=None):
        ...
        for _ in range(n_evals):
            ...
            if isinstance(result, tuple):
                state = result[0]  # 新版本 gym 返回 (observation, info)
            else:
                state = result      # 旧版本 gym 直接返回 observation
            while not done:
                time_step += 1
                ...
                try:
                    ...
                    if len(result) == 5:
                        # 新版本 gym 返回 (obs, reward, terminated, truncated, info)
                        state, reward, terminated, truncated, info = result
                        done = terminated or truncated
                    else:
                        # 旧版本 gym 返回 (obs, reward, done, info)
                        state, reward, done, info = result
                except ValueError:
                    # 如果解包失败，可能是版本兼容性问题，尝试不同的方式
                    result = test_env.step(action)
                    if len(result) == 5:
                        state, reward, terminated, truncated, info = result
                        done = terminated or truncated
