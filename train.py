...
# 在导入 d4rl 之前先导入 mujoco_py 确保 MuJoCo 可用
import mujoco_py
...
from offline_piro_trainer import OfflinePiroTrainer  # Import new PIRO trainer
...
def train_agent_piro(params, online_yaml_config):
    """
    Train agent using Simplified Offline PIRO algorithm with constraint-based reward updates
    """
    # Create environment
    ...
    agent = SAC_Agent(params['seed'], state_dim, action_dim, **params.get('sac_params', {}))
    
    print(f"Training agent using Simplified Offline PIRO on {params['env_name']}")
    print(f"State dim: {state_dim}, Action dim: {action_dim}, Seed: {params['seed']}")
    
    # PIRO parameters with constraint settings (based on irl_samples.py)
    piro_params = params.copy()
    piro_params.update({
        'piro_outer_loops': params.get('piro_outer_loops', 200),      # m: fewer outer loops 
        'piro_policy_rounds': params.get('piro_policy_rounds', 5),   # k: policy updates per loop
        'piro_reward_rounds': params.get('piro_reward_rounds', 3),   # n: reward updates per loop
        
        # Constraint parameters from irl_samples.py approach
        'piro_target_reward_diff': params.get('piro_target_reward_diff', 0.1),
        'piro_target_ratio_upper': params.get('piro_target_ratio_upper', 1.2),
        'piro_target_ratio_lower': params.get('piro_target_ratio_lower', 0.8),
        'piro_coef_scale_down': params.get('piro_coef_scale_down', 0.9),
        'piro_coef_scale_up': params.get('piro_coef_scale_up', 1.1),
        'piro_coef_min': params.get('piro_coef_min', 0.001),
        'piro_coef_max': params.get('piro_coef_max', 10.0),
        'piro_target_reward_l2_norm': params.get('piro_target_reward_l2_norm', 1.0),
        'piro_l2_coef_scale_up': params.get('piro_l2_coef_scale_up', 1.1),
        'piro_l2_coef_scale_down': params.get('piro_l2_coef_scale_down', 0.9),
        
        'piro_reward_lr': params.get('piro_reward_lr', 3e-4),
        'piro_reward_hidden': params.get('piro_reward_hidden', [256, 256]),
    })
    
    # Create PIRO trainer
    trainer = OfflinePiroTrainer(piro_params, env, agent, device=device)
    
    # Run simplified PIRO training
    rewards = trainer.train_offline_piro(
        save_model=True,
        save_policy=True,
        load_model_dir=params.get('load_model_dir', None)
    )
    
    print(f"Simplified PIRO training completed. Final reward statistics:")
    if rewards:
        print(f"  Mean reward over training: {np.mean(rewards):.2f}")
...
def train_agent_new(params, online_yaml_config=None):
    ...
    # 对于旧版本的 gym，需要分别调用 seed() 和 reset()
...
def main():
    ...
    # PIRO algorithm parameters
    parser.add_argument('--use_piro', dest='use_piro', action='store_true', 
                        help='Use Offline PIRO algorithm instead of original ML-IRL')
    parser.add_argument('--piro_outer_loops', type=int, default=200,
                        help='Number of outer loops (m) in PIRO algorithm')
    parser.add_argument('--piro_policy_rounds', type=int, default=5,
    ...
    parser.set_defaults(use_piro=False)
    ...
    # Choose training algorithm based on parameters
    if params['use_piro']:
        print("Using Offline PIRO algorithm with constraint-based reward updates")
        rewards = train_agent_piro(params, online_yaml_config)
    else:
        print("Using original ML-IRL algorithm")
        rewards = train_agent_new(params, online_yaml_config)
        
    rewards = np.array(rewards)
    print(f"Training completed. Reward statistics:")
