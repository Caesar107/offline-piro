...
from utils import MeanStdevFilter, prepare_data, reward_func, GaussianMSELoss, truncated_normal_init, Transition, \
    ReplayPool, check_or_make_folder, FasterReplayPool, parameterized_truncated_normal
...
class EnsembleGymEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # 处理 current_state 可能是 tuple 的情况
        if isinstance(self.current_state, tuple):
            state_for_predict = self.current_state[0].reshape(1, -1)
        else:
            state_for_predict = self.current_state.reshape(1, -1)
    ...
    def reset(self):
        ...
        if isinstance(result, tuple):
            self.current_state = result[0]  # 新版本 gym 返回 (observation, info)
        else:
            self.current_state = result      # 旧版本 gym 直接返回 observation
        self._elapsed_steps = 0
